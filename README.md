...
- 安全停机余量台账：按程序轨迹最高速度、控制器响应时间和制动减速度结算停机距离与路径最小间距的余量；响应时间与减速度多来源取最保守值，任一来源缺失标为待复核且不覆盖已有有效结论；同一输入幂等只生成一条台账，历史台账只读。唯一执行入口为 `POST /api/v1/stop-margin-ledgers`（`safety_engineer`/`admin`），只读查询 `GET /api/v1/stop-margin-ledgers[/:id]`。
